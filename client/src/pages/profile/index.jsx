  ...
  const handleDeleteImage = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        toast.success("Profile image removed sucessfully");
        setImage(null);
      }
    } catch (error) {
      console.log(error);
    ...
    <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        ...
        <div className="grid grid-cols-2">
          ...
          >
            <Avatar className="w-32 h-32 md:w-32 md:h-32 rounded-full overflow-hidden">
              {image ? (
                <AvatarImage
                  src={image}
                  alt="profile"
                ...
                <div
                  className={`uppercase h-32 w-32 md:w-32 md:h-32 text-5xl border-[1px] flex items-center justify-center rounded-full ${getColor(
                    selectedColor
                  )} `}
                >
                  {firstName
